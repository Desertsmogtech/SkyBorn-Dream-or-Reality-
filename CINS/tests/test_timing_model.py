...
class TestDeadlineMonitor(unittest.TestCase):
    ...
    def test_violations_accumulate(self):
        ...
        wm.register({"task_id": "dm004", "tier": "soft-rt",
                      "budget_ms": 50, "jitter_ms": 5, "assigned_at": past})
        overruns = monitor.check_all()
        self.assertEqual(len(overruns), 2)
        ids = [o["task_id"] for o in overruns]
        self.assertIn("dm003", ids)
        self.assertIn("dm004", ids)

    def test_fresh_window_not_flagged(self):
        monitor, wm = self._make_monitor()
        wm.register({
            "task_id": "dm005",
            "tier": "best-effort",
        ...
        overruns = monitor.check_all()
        task_ids = [o["task_id"] for o in overruns]
        self.assertNotIn("dm005", task_ids)
